# Contents of ADS2 Yiru Exam Skill Bundle

Access: GitHub: [Open GitHub folder](https://github.com/CHENyiru3/awesome_ZJE_resource/tree/main/resources/COURSES/Year2/ADS2/Yiru/ADS2_Yiru_exam_skill_bundle) | OneDrive: [Browse Year 2 folder](https://zjuintl-my.sharepoint.com/:f:/g/personal/yiru_22_intl_zju_edu_cn/IgAQsSutUsB9SKby83Q5DBL8ARaH6G0SwoRC1PcAGeImmn4?e=c7kbO5)

Portable Markdown skill bundle for ADS2 online problem-solving exams and ICA-style analysis.

## Included Skills

- `ads2-exam-workflow`
- `ads2-data-cleaning`
- `ads2-probability-bayes-markov`
- `ads2-power-sample-size`
- `ads2-t-wilcoxon-tests`
- `ads2-anova-factorial`
- `ads2-categorical-tests`
- `ads2-correlation-regression`
- `ads2-simulation-bootstrap`
- `ads2-clustering-feature-extraction`
- `ads2-ica-open-analysis`
- `ads2-result-interpretation`
- `ads2-common-traps-checklist`

## Files

- `README.md`
- `source-index.md`
- `skills/*/SKILL.md`
